    ...
    headers: {
      ...
      const postdata = async (data) => {
        const response = await axios.post(
          `https://nextapplicatin.iran.liara.run/api/update-slider/${row.getValue(
            "_id"
          )}`,
          data,configss
        );
      ...
      const deletedata = async () => {
        const response = await axios.post(
          `https://nextapplicatin.iran.liara.run/api/delete-slider/${row.getValue(
            "_id"
          )}`,{item:1},configss
        );
        return response;
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            >
              ...
              />
                  <input type="number"
                defaultValue={row.getValue("sorter")}
                className="p-[10px] rounded-md text-[14px] outline-none border-[1px] border-zinc-500 w-[90%]"
                {...register("sorter", { required: true })}
              />
              <select
